...
public class EmployeeManager {
    ...
    public static Employee addPartEmployee() {
        ...
        return new PartTimeEmployee(IdRegex.getId(), name, AgeRegex.getAge(), GenderRegex.getGender(), PhoneRegex.getPhone(), EmailRegex.getEmail(), StatusRegex.getStatus(), SalaryRegex.getSalary(), TimeWorkingRegex.getTimeWorking());
    }
    public static Employee addFullEmployee() {
        System.out.print("Nhập tên nhân viên Fulltime: ");
        String name = scanner.nextLine();
        return new FullTimeEmployee(IdRegex.getId(), name, AgeRegex.getAge(), GenderRegex.getGender(), PhoneRegex.getPhone(), EmailRegex.getEmail(), StatusRegex.getStatus(), SalaryRegex.getSalary());
    }
    ...
    public static void showFull() {
        int index = -1;
        for(int i = 0; i < list.size(); i++){
            if (list.get(i).toString().contains("FullTimeEmployee")) {
                index = i;
                System.out.println(list.get(i).toString());
            }
        }
    ...
    public static void showPart() {
        int index = -1;
        for(int i = 0; i < list.size(); i++){
            if (list.get(i).toString().contains("PartTimeEmployee")) {
                index = i;
                System.out.println(list.get(i).toString());
            }
        }
